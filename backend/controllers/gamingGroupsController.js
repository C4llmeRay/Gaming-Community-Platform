...
exports.transferOwnership = async (req, res) => {
  try {
    ...
    if (group.owner.toString() !== currentOwner._id.toString()) {
      return res.status(400).json({ message: 'Only the owner can transfer ownership' });
    }

    // Check if the new owner is a member of the group
    if (!group.members.includes(newOwnerId)) {
      return res.status(400).json({ message: 'The new owner must be a member of the group' });
    }
...
exports.kickMember = async (req, res) => {
  try {
    ...
    const user = req.user;
    if (group.owner.toString() !== user._id.toString() && !group.moderators.includes(user._id.toString())) {
...
exports.promoteMember = async (req, res) => {
  try {
    ...
    if (group.owner.toString() !== user._id.toString()) {
      return res.status(403).json({ message: 'Only the group owner can promote members' });
    }
    ...
    res.status(200).json({ message: 'Member has been promoted to a moderator' });
  } catch (error) {
...
exports.demoteMember = async (req, res) => {
  try {
    ...
    const user = req.user;
    if (group.owner.toString() !== user._id.toString()) {
      return res.status(403).json({ message: 'Only the group owner can demote members' });
    }
    ...
    group.moderators = group.moderators.filter((moderator) => moderator.toString() !== memberId);
    await group.save();

    res.status(200).json({ message: 'Member has been demoted to a regular member' });
  } catch (error) {
